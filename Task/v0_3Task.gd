...
var codex:EB_Codex=preload("res://Task/codex.tres")
...
func refresh():
	...
	for i in bag.get_all_items():
		...
		var tag:=EB_ItemTag.new()
		tag.tag_id="tag_1"
		if i.has_tag_by_id(tag.tag_id):
			print(str(i.get_item_name())+"is a food")
			itemNode.text=itemNode.text+" is food"
		else:
			print(str(i.get_item_name())+"not a food")
